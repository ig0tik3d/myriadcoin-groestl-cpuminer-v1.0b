...
typedef struct {
SHA256_CTX sha1;
} Myriad_context_holder;

Myriad_context_holder base_contexts;

void init_Myriad_contexts(int mode)
{ 
	SHA256_Init(&base_contexts.sha1);
}



static void MyriadHash(void *state, const void *input,int mode)
{
	uint32_t hash1[16];
	...
	SHA256_CTX sha256;
	memcpy(&sha256,&base_contexts.sha1,sizeof(SHA256_CTX));
...
//	}
	...
	if (ptarget[7]==0) {
		do {
			...
			MyriadHash(hash64, &endiandata,alg);
